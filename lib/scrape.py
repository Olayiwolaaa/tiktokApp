...
def usernames_n_links(file_name, hashtags, data, number_of_scrolls=3):
    ...
    for tag in tqdm(hashtags, desc="Processing hashtags"):
        try:
            ...
            # Scroll down 3 times (was previously 10, reduced for testing purposes)
            ...
            for user_link in soup.find_all('a', class_='tiktok-nafpq5-StyledLink exdlci12'):
                user_href = user_link.get("href")
                user_profile_link = f'https://www.tiktok.com{user_href}'
    ...
    for hashtag in data:
        ...
        for user in users:
            if 'email' in data[hashtag][user]:
                continue
            try:
                response = requests.get(data[hashtag][user]["link"], headers=headers, cookies=cookies)
                soup = BeautifulSoup(response.content, 'html.parser')
                num_datas_scraped += 1

                # Extract user bio from the parsed HTML
                h2_tag = soup.find('h2', class_='tiktok-1n8z9r7-H2ShareDesc e1457k4r4')
                if h2_tag is None:
                ...
                email_match = re.search(email_regex, h2_tag.get_text())
